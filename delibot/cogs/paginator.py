...
class HelpPaginator(Pages):
    ...
    async def show_page(self, page, *, first=False):
        ...
        #    value = 'Delibot Help!'
        #    self.embed.add_field(name='Delibot', value=value, inline=False)
        ...
        for entry in entries:
            if len(entry.aliases) > 0:
                if entry.help is not None:
                    value_desc = entry.help + "\n **Aliases:** " + ", ".join(entry.aliases) or "No help given"
                else:
                    value_desc = "No help given" + "\n **Aliases:** " + ", ".join(entry.aliases)
            else:
                value_desc = entry.help or "No help given"

            # Custom hide args
            if signature(entry) == "exraid <pokemon> <time> <day> <location> [delete=True]":
                fixed_entry = ":small_blue_diamond: exraid <pokemon> <time> <day> <location>"
                self.embed.add_field(name=fixed_entry, value=value_desc, inline=False)
            elif signature(entry) == "community_day [region] [GMT]":
                fixed_entry = ":small_blue_diamond: community_day"
                self.embed.add_field(name=fixed_entry, value=value_desc, inline=False)
            else:
                self.embed.add_field(name=":small_blue_diamond: " + signature(entry), value=value_desc, inline=False)
