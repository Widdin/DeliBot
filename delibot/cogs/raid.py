...
class Raid(commands.Cog):
    ...
    async def ask_for_input(self, ctx):
        ...
        embed = discord.Embed(title=thank_you,
                              description=f"{raid_creation} [{ctx.message.channel.name}]({channel_url})",
    ...
    async def raid(self, interaction: discord.Interaction, pokemon: str, time: str, location: str) -> None:
        """
        ...
        (default_channel,) = await self.bot.db.get_default_channel(interaction.guild_id)

        # Create the user in the database if he doesn't exist.
        # await self.bot.get_cog("Utils").create_user_if_not_exist(interaction.guild_id, interaction.message.author.id)
        ...
        params = (
            str(interaction.guild_id), str(raid_message.channel.id), str(raid_message.id), str(interaction.user.id), "",
            str(pokemon), str(time), str(location.lower()), "", "", "")
        ...
        # Send Logs
        log_channel_id = await self.bot.get_cog("Utils").get_log_channel(interaction.guild_id)
        if log_channel_id is not None:
            embed = discord.Embed(title="[RAID] - Created",
                                  description=f'➥ :bust_in_silhouette: {interaction.message.author.mention} ({interaction.message.author})\n➥ :mag_right: {pokemon} ({time}) @ {gym_name} ([{raid_message.id}](https://discordapp.com/channels/{interaction.message.guild.id}/{raid_message.channel.id}/{raid_message.id}))',
